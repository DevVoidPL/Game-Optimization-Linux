"""Qt application bootstrap for GameForge Linux."""
...
def _extract_runtime_options(
    ...
    """Remove GameForge-only flags before Qt parses its own arguments."""
    ...
    if not qt_arguments:
        qt_arguments.append("gameforge-linux")
...
def _install_termination_handlers(
    ...
    def request_quit(signum: int, _frame: FrameType | None) -> None:
        if requested[0] is None:
            requested[0] = signum
            logger.info("Received signal %s; stopping GameForge", signum)
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    mode = (
        "Demo"
        if os.environ.get("GAMEFORGE_DEMO", "").strip() == "1"
    ...
    try:
        ...
        logger.info("Keyboard interrupt received; stopping GameForge")
        return 130
    except Exception:
