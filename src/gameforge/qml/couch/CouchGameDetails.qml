...
FocusScope {
    ...
    focus: visible
    Component.onCompleted: { loadMangoHudProfile(); loadOptimizationProfile(); loadOptiScalerStatus(); restoreActiveFocus() }
    onVisibleChanged: if (visible) { loadMangoHudProfile(); restoreActiveFocus() }
    onGameChanged: { loadMangoHudProfile(); loadOptimizationProfile(); loadOptiScalerStatus() }

    Connections {
        target: page.controller || null
        ignoreUnknownSignals: true
        function onOptiScalerChanged(appId) {
            if (String(appId) === String(page.optiScalerData.appId || ""))
