...
def render_desktop_entry() -> str:
    return (
        "# Generated from gameforge.config; do not edit the name here.\n"
        "[Desktop Entry]\n"
        "Type=Application\n"
        "Version=1.0\n"
        f"Name={APP_NAME}\n"
        "Comment=Manage and inspect local Linux game libraries\n"
        "Exec=gameforge-linux\n"
        f"Icon={APP_ID}\n"
        "Terminal=false\n"
        "Categories=Game;\n"
        "StartupNotify=true\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        print("usage: python -m gameforge.desktop_entry OUTPUT.desktop", file=sys.stderr)
        return 2
