...
HOST_COMPONENT_DIRECTORY = Path(".local/share/gameforge-linux/bin")
...
def default_runner_path(
    ...
    return host_home_directory(environment) / HOST_COMPONENT_DIRECTORY / "gameforge-run"
...
def bootstrap_flatpak_host_components(
    ...
    *,
    ...
    sources = {
        "gameforge-run": Path(app_prefix) / "libexec/gameforge-run-host",
    }
    changed = tuple(
        name
