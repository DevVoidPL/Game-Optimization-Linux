...
from gameforge.config import GAMES_CONFIG_DIR, MANGOHUD_LOG_DIR
...
MANAGED_HEADER = "# Managed by GameForge Linux"
...
# Every key emitted by GameForge is present in MangoHud 0.8.4's distributed
...
class MangoHudProfileRepository:
    """Own only GameForge's per-AppID JSON files under XDG config."""
...
class MangoHudConfigWriter:
    ...
    def render(self, profile: MangoHudProfile) -> str:
        ...
        lines = [
            ...
            "# This file is managed by GameForge; global MangoHud files are untouched.",
            "",
        ]
...
class MangoHudLaunchIntegration:
    ...
    @staticmethod
    def _managed_by_gameforge(path: Path, app_id: str) -> bool:
        if path.is_symlink():
            return False
        try:
    ...
    def _remove_owned_application_config(
        ...
        if path is None or not self._managed_by_gameforge(path, profile.app_id):
            return
        try:
            path.unlink(missing_ok=True)
        except OSError:
    ...
    def status(
        ...
        if (target.exists() or target.is_symlink()) and not self._managed_by_gameforge(
            target, profile.app_id
        ):
            return MangoHudApplicationConfigStatus(
                "steam_environment", "application_config_conflict",
                "An existing MangoHud application configuration is not managed by GameForge",
    ...
    def reset(self, game: Game) -> None:
        """Remove only the per-game Flatpak mirror managed by GameForge."""
        ...
        try:
            ...
            # A non-empty directory may contain future GameForge-owned artifacts.
