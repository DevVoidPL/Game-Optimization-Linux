...
if [ "${FLATPAK_ID:-}" = "$app_id" ]; then
    ...
    install_root="$HOME/.local/share/gameforge-linux"
else
    install_root="${XDG_DATA_HOME:-$HOME/.local/share}/gameforge-linux"
fi
python_root="$install_root/python"
runner_root="$install_root/bin"
mkdir -p "$runner_root"

temporary_runner="$(mktemp "$runner_root/.gameforge-run.XXXXXX")"
...
if [ "${FLATPAK_ID:-}" = "$app_id" ] \
    ...
    {
        printf '%s\n' '#!/usr/bin/env sh'
        printf '%s\n' 'exec flatpak run --command=gameforge-run io.github.gameforge_linux.GameForge "$@"'
    } > "$temporary_runner"
else
    mkdir -p "$python_root"
    ...
    {
        printf '%s\n' '#!/usr/bin/env sh'
        printf 'PYTHONPATH=%s exec python3 -m gameforge.runner "$@"\n' "$(printf '%q' "$python_root")"
    } > "$temporary_runner"
fi
chmod 0755 "$temporary_runner"
mv -f "$temporary_runner" "$runner_root/gameforge-run"
trap - EXIT

printf 'Installed GameForge Runner: %s\n' "$runner_root/gameforge-run"
